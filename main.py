...
async def play(interaction: discord.Interaction, query: str):
    ...
    try:
        if not interaction.response.is_done():
            ...
            print(
                f"⚠️ /play interaction already acknowledged: "
                f"id={interaction.id}"
            )
    except discord.NotFound:
        print(
            f"❌ /play interaction expired: id={interaction.id}"
        )
        return
    except discord.HTTPException as exc:
        # Discord error 40060 means another response already acknowledged
        # this interaction. Continue using followup instead of crashing.
        if getattr(exc, "code", None) == 40060:
            print(
                f"⚠️ /play interaction already acknowledged by Discord: "
                f"id={interaction.id}"
            )
        else:
            print(
                f"❌ /play acknowledgement failed: "
                f"{type(exc).__name__}: {exc}"
            )
            return

    # Voice check uses followup if the interaction is already acknowledged.
...
async def volumedown(interaction: discord.Interaction):
    """Decrease playback volume safely."""

    # V21.1: Validate guild, voice state, source, and volume bounds.
    guild_id = interaction.guild_id
    ...
    try:
        ...
        print(
            f"⚠️ /volumedown failed in guild {guild_id}: "
            f"{type(exc).__name__}: {exc}"
        )
        try:
            await interaction.response.send_message(
                "❌ Failed to decrease the volume. Please try again.",
                ephemeral=True,
            )
        except discord.InteractionResponded:
            await interaction.followup.send(
        ...
        print(
            f"⚠️ /volumedown invalid source volume in guild {guild_id}: "
            f"{type(exc).__name__}: {exc}"
        )
        try:
            await interaction.response.send_message(
        ...
        print(
            f"❌ /volumedown unexpected error in guild {guild_id}: "
            f"{type(exc).__name__}: {exc}"
        )
        try:
            await interaction.response.send_message(
                "❌ Volume decrease failed. Please try again.",
                ephemeral=True,
            )
        except Exception:
            pass
